...
(defun init-dyci2-lib (path)
  (if (and path (probe-file path) (stringp (pathname-name path)))
      (let ((folder (make-pathname :directory (pathname-directory path)
                                   :device (pathname-device path)
                                   :host (pathname-host path)))
            (name (pathname-name path)))
        (princ (format nil "Loading DYCI2 from ~s~&" path))
        (setf *dyci2-dict* (Dyci2Init (namestring folder) name))
        )
    (print "Warning: DYCI2 load module not found!"))
